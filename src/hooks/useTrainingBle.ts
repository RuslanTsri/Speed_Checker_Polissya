...
import { BleManager, Device, Characteristic, BleError, ScanMode } from 'react-native-ble-plx';
...
export const useTrainingBle = () => {
    ...
    useEffect(() => {
        const subscription = bleManager.onStateChange((bleState) => {
            if (bleState === 'PoweredOff') {
                Alert.alert('Bluetooth вимкнений', 'Будь ласка, увімкніть Bluetooth');
            }
        }, true);
    ...
    const scanForDevices = async () => {
        ...
        if (!hasPerms) {
            Alert.alert('Помилка', 'Немає прав доступу до Bluetooth');
            return;
        }
        ...
        scanTimeoutRef.current = setTimeout(() => {
            if (!connected && !isConnecting.current) {
                stopScanning();
                setState('idle');
                Alert.alert('Пристрій не знайдено', 'Перевірте живлення Master Node');
            }
        }, 15000);
    };
    ...
    const handleMasterResponse = (data: any) => {
        switch (data.type) {
            ...
            case 22:
                ...
                } else if (data.sensor !== undefined) {
                    updateSensorStatus(data.sensor, data.status === 'OK' ? 'active' : 'timeout', data.rssi);
                    setPingProgress(`Сенсор ${data.sensor}...`);
                }
                break;
            case 30:
    ...
    const handleTrigger = (data: any) => {
        setSensors(prev => prev.map(s =>
            s.id === data.sensor
                ? { ...s, status: 'active', triggerTime: data.time, splitTime: data.split }
                : s
        ));
        setSession(prev => {
            if (!prev) return null;
            return {
                ...prev,
                triggers: [...prev.triggers, { sensorId: data.sensor, time: data.time, split: data.split }],
            };
        });
    };
    ...
    const sendCommand = async (command: CommandType) => {
        ...
        try {
            ...
            console.error('[COMMAND] Error sending');
        }
    };
    ...
    const startDiscovery = async () => {
        if (connected) {
            setState('discovering');
            setPingProgress('Pinging...');
            isPinging.current = true;
            // Скидаємо статуси (крім Master)
    ...
    const startTraining = () => {
        ...
        if (count === 0) {
            Alert.alert("Увага", "Не знайдено активних сенсорів");
            return;
        }
