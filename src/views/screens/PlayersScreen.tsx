...
import { IconButton, Button } from "../components/ui/Button"; // 🔥 Імпорт твого Button
import { TextField } from '../components/ui/TextField';       // 🔥 Імпорт твого TextField
import {
    TeamsIcon, TeamsIconActive,
    ArrowIcon, ArrowIconActive,
    DocIcon, DocIconActive,
    UserIcon, UserIconActive,
    ImportIcon, ImportIconActive,   // 🔥 Нові іконки
    ExportIcon, ExportIconActive, PenIcon, PenIconActive   // 🔥 Нові іконки
} from '../../../assets/icons';
...
export default function PlayersScreen() {
    ...
    const {
        filteredTeams, players, selectedTeam, setSelectedTeam,
        ...
        return (
            <View className="flex-1 pt-4 relative">
                ...
                <Modal visible={isDropdownVisible} transparent animationType="fade">
                    <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setDropdownVisible(false)}>
                        <View className="absolute top-16 right-4 rounded-2xl border border-white/10 shadow-2xl overflow-hidden min-w-[180px] bg-[#0A0A0A]/95">
                            ...
                            {/* 🔥 Редагувати команду (Замінили на Pressable та PenIcon) */}
                            <Pressable
                            ...
                            >
                                {({ pressed }) => (
                                    <>
                                        ...
                                        <Text className="font-medium text-sm text-[#F5F5F5]" style={{ fontFamily: 'Evolventa' }}>
                            ...
                            {/* Видалити команду (Залишаємо Feather, щоб червона корзина виділялася) */}
                            <TouchableOpacity onPress={() => handleDeleteTeam()} className="flex-row items-center px-4 py-4 active:bg-red-500/10">
                                <Feather name="trash-2" size={18} color="#ef4444" style={{ marginRight: 12 }} />
                                <Text className="text-red-500 font-medium text-sm" style={{ fontFamily: 'Evolventa' }}>
                ...
                <View className="flex-row justify-between px-6 pb-4 border-b border-white/5 mb-4">
                    <Text className="text-[10px] font-bold tracking-widest uppercase text-[#A3A3A3]" style={{ fontFamily: 'Evolventa' }}>
                        {t('screens.players.label_players_count', { count: players.length })}
                    </Text>

                </View>

                {/* Кнопка "Додати гравця" */}
                ...
                <AppModal type="center" visible={isDeleteTeamModalVisible} onClose={() => setDeleteTeamModalVisible(false)} title={t('screens.players.modal_delete_title')}>
                    <View className="items-center mb-6 mt-2">
                        ...
                        <Text className="text-lg font-bold text-center mb-2 text-white" style={{ fontFamily: 'Unbounded' }}>
                            Видалити команду?
                        </Text>
                        <Text className="text-center text-sm px-4 text-[#A3A3A3]" style={{ fontFamily: 'Evolventa' }}>
                            Ви впевнені, що хочете видалити команду <Text className="text-white font-bold">{selectedTeam?.name}</Text>? Всі гравці та їх результати будуть втрачені.
                        </Text>
                    </View>
                        ...
                        >
                            {({ pressed }: { pressed: boolean }) => (
                                <>
                                    ...
                                    <View>
                                        <Text className="text-[#F5F5F5] text-sm font-bold leading-5 mb-1" style={{ fontFamily: 'Unbounded' }}>{t('screens.players.import_from_file')}</Text>
                                        <Text className="text-[#A3A3A3] text-[10px] leading-4" style={{ fontFamily: 'Evolventa' }}>{t('screens.players.import_desc_csv')}</Text>
                                    </View>
                                </>
                            )}
                        </Pressable>
                        ...
                        >
                            {({ pressed }: { pressed: boolean }) => (
                                <>
                                    ...
                                    <View>
                                        <Text className="text-[#F5F5F5] text-sm font-bold leading-5 mb-1" style={{ fontFamily: 'Unbounded' }}>{t('screens.players.add_manual')}</Text>
                                        <Text className="text-[#A3A3A3] text-[10px] leading-4" style={{ fontFamily: 'Evolventa' }}>{t('screens.players.add_manual_desc')}</Text>
                                    </View>
                                </>
                            )}
                        </Pressable>
                ...
                <AppModal type="center" visible={isDeletePlayerModalVisible} onClose={() => setDeletePlayerModalVisible(false)} title={t('screens.players.modal_delete_title')}>
                    <View className="items-center mb-6 mt-2">
                        ...
                        <Text className="text-lg font-bold text-center mb-2 text-white" style={{ fontFamily: 'Unbounded' }}>{t('screens.players.delete_player_confirm_title')}</Text>
                        <Text className="text-center text-sm px-4 text-[#A3A3A3]" style={{ fontFamily: 'Evolventa' }}>{t('screens.players.delete_player_confirm_desc', { name: playerToDelete?.name })}</Text>
                    </View>
                    <View className="flex-row gap-3">
                        {/* 🔥 Button для скасування, залишили червону кнопку для видалення */}
                    ...
                    {importedPlayers.length === 0 ? (
                        <>
                            ...
                            <Pressable
                                ...
                                style={({ pressed }: { pressed: boolean }) => [pressed && { opacity: 0.7, transform: [{ scale: 0.98 }] }]}
                            >
                                {({ pressed }: { pressed: boolean }) => (
                                    <>
                                        ...
                                        <View className="flex-1">
                                            <Text className="font-bold text-base text-white" style={{ fontFamily: 'Unbounded' }}>{t('screens.players.import_template')}</Text>
                                            <Text className="text-xs text-[#A3A3A3]" style={{ fontFamily: 'Evolventa' }}>{t('screens.players.import_template_desc')}</Text>
                                        </View>
                            ...
                            >
                                {({ pressed }: { pressed: boolean }) => (
                                    <>
                                        ...
                                        <Text className="text-[#FF6D00] font-bold" style={{ fontFamily: 'Evolventa' }}>
                        ...
                        // 🔥 СТАН КОЛИ ФАЙЛ ЗАВАНТАЖЕНО (Як на скріні)
                        <View className="w-full">
                            ...
                            <View className="flex-row items-center border border-emerald-500/30 bg-[#0A0A0A]/50 p-4 rounded-2xl mb-6 shadow-sm">
                                ...
                                <View className="flex-1">
                                    <Text className="font-bold text-base text-[#F5F5F5] mb-0.5" style={{ fontFamily: 'Unbounded' }}>
                                        Файл перевірений
                                    </Text>
                                    <Text className="text-xs text-emerald-500 font-medium" style={{ fontFamily: 'Evolventa' }}>
                            ...
                            {/* Заголовок списку */}
                            <Text className="text-[11px] font-bold text-[#F5F5F5] mb-3" style={{ fontFamily: 'Evolventa' }}>
                                Список гравців
                            </Text>
    ...
    return (
        <View className="flex-1 pt-4 relative">
            <View className="flex-row items-center justify-between px-4 mb-6 relative z-10">
                <Text className="text-3xl font-bold flex-1 text-[#F5F5F5]" style={{ fontFamily: 'Unbounded' }}>
                            ...
                            <View className="flex-row gap-2 mt-1">
                                <View className="flex-row items-center px-2 py-1 rounded-md border border-white/10 bg-white/5">
                                    ...
                                    <Text className="text-[10px] font-bold text-[#DCDCDC]">
                                        {t('screens.players.label_players_count_short', { count: item.playerCount })}
                                    </Text>
                                </View>
                                {item.isMyTeam && (
            ...
            {/* Створення нової команди (на весь екран) */}
