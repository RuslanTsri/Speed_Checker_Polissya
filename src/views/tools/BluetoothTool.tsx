...
// --- PulseRing (Стандартний Animated, без Reanimated) ---
...
export default function BluetoothTool({ onBack }: BluetoothToolProps) {
    const {
        ...
        state,
        sensors,
        elapsedTime, // Використовуємо час з твого хука
        pingProgress,
        startDiscovery,
        startTraining,
        stopTraining,
        ...
        <View className="flex-1 px-4 pt-4 bg-slate-900">
            ...
            <View className={`p-6 rounded-3xl border items-center shadow-lg mb-4 ${connected ? 'bg-slate-800 border-green-500/30' : 'bg-slate-800 border-slate-700'}`}>
                ...
                {state === 'discovering' ? (
                    ...
                    <>
                        <Text className="text-white text-2xl font-bold mb-1">{connected ? 'ПІДКЛЮЧЕНО' : 'ВІДКЛЮЧЕНО'}</Text>
                        <Text className="text-slate-400 text-xs mb-4">STM32 Master Node</Text>
                        {!connected ? (
                            <TouchableOpacity onPress={startDiscovery} className="bg-yellow-400 px-10 py-3 rounded-xl shadow-lg shadow-yellow-400/20 active:bg-yellow-500">
                                <Text className="text-slate-900 font-bold text-lg">ПІДКЛЮЧИТИСЯ</Text>
                            </TouchableOpacity>
                        ) : (
                            <TouchableOpacity onPress={handleDisconnect} className="flex-row items-center bg-slate-700/50 border border-slate-600 px-4 py-2 rounded-full active:bg-slate-700">
            ...
            {connected && state !== 'discovering' && (
                <View className="absolute bottom-6 left-4 right-4 gap-3">
                    ...
                    {state === 'idle' || state === 'ready' ? (
                        ...
                        // ВАРІАНТ 2: ACTIVE (БІЖИМО) -> Великий СТОП
                        ...
                        // ВАРІАНТ 3: ARMED (ЧЕКАЄМО) або FINISHED (ФІНІШ) -> СТОП + СКИНУТИ
                        <View className="flex-row gap-3">
                            {/* Ліва кнопка: СТОП (для відміни Armed або повторного стопу) */}
                            <TouchableOpacity onPress={stopTraining} className="flex-1 bg-red-500/20 border border-red-500 py-4 rounded-xl items-center active:bg-red-500/30">
                                <Text className="text-red-400 font-bold uppercase">СТОП</Text>
                            </TouchableOpacity>

                            {/* Права кнопка: СКИНУТИ (Новий заїзд) */}
                            <TouchableOpacity onPress={resetSession} className="flex-[2] bg-slate-700 py-4 rounded-xl items-center active:bg-slate-600">
