...
export default function SessionsTeam({ searchQuery, onSelectSession }: Props) {
    ...
    if (isLoading && teamSessions.length === 0) {
        return <View className="mt-10"><ActivityIndicator color={isDark ? "#facc15" : "#eab308"} /></View>;
    }
    ...
    return (
        <FlatList
            ...
            onRefresh={refresh}
            ListEmptyComponent={() => (
                <Text className={`text-center mt-10 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>Команд не знайдено</Text>
            )}
            renderItem={({ item }) => (
                <TouchableOpacity
                    activeOpacity={0.8}
                    ...
                    <View className="flex-row justify-between items-start mb-2">
                        ...
                        {item.hasResults ? (
                            <View className={`flex-row items-center px-2 py-1 rounded-lg border ${isDark ? 'bg-green-500/10 border-green-500/20' : 'bg-green-50 border-green-200'}`}>
                                ...
                                <Text className={`text-[10px] font-bold uppercase ${isDark ? 'text-green-400' : 'text-green-600'}`}>Є дані</Text>
                            </View>
                        ) : (
                            <View className={`flex-row items-center px-2 py-1 rounded-lg border ${isDark ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
                                ...
                                <Text className={`text-[10px] font-bold uppercase ${isDark ? 'text-red-400' : 'text-red-600'}`}>Немає даних</Text>
                            </View>
                        )}
                    </View>
