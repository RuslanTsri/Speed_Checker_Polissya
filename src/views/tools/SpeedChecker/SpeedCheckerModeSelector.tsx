...
export default function SpeedCheckerModeSelector({ onBack, onSelect }: Props) {
    ...
    let statusTitle = "Не підключено";
    let statusSubtitle = "Перевірте Bluetooth";
    let statusBg = "bg-red-900/20";
    let statusBorder = "border-red-500/30";
    let statusTextCol = "text-red-400";

    if (connected) {
        if (pingProgress) {
            statusTitle = pingProgress;
            statusSubtitle = "Зачекайте...";
            statusBg = "bg-yellow-900/20";
            statusBorder = "border-yellow-500/30";
            statusTextCol = "text-yellow-400";
        } else {
            statusTitle = "Готово до тесту";
            statusSubtitle = `Конфігурація: ${activeSensorsCount} гейт(и/ів)`;
    ...
    return (
        <View className="flex-1 bg-slate-950 pt-4 px-4">
            ...
            <View className={`${statusBg} border ${statusBorder} p-5 rounded-2xl flex-row justify-between items-center mb-8`}>
                <View className="flex-1 mr-2">
                    <Text className={`${statusTextCol} font-bold text-base mb-1`}>
                        {statusTitle}
                    </Text>
            ...
            <View className="bg-slate-900 p-1 rounded-2xl flex-row mb-8 border border-slate-800">
                <TouchableOpacity
                    ...
                    className={`flex-1 py-3 rounded-xl flex-row justify-center items-center ${mode === 'DEVICE' ? 'bg-yellow-400' : ''}`}
                >
            ...
            <TouchableOpacity
                onPress={() => onSelect(mode, 'QUICK')}
                activeOpacity={0.8}
                className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex-row items-center mb-4 active:bg-slate-800/80"
            >
            ...
            <TouchableOpacity
                onPress={() => onSelect(mode, 'TEAM')}
                activeOpacity={0.8}
                className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex-row items-center active:bg-slate-800/80"
            >
                ...
                <View className="flex-1">
                    <Text className="text-white text-xl font-bold mb-1">Тест команди
                    </Text>
