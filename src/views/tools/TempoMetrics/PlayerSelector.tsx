        ...
        return (
            <View className="flex-1 px-4 pt-4">
                <View className="flex-row items-center justify-between mb-6">
                    ...
                    <Text className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Команда пуста</Text>
                    <View className="w-10" />
                </View>
                <View className="items-center justify-center flex-1 mb-20">
                    <Text className={`font-bold uppercase tracking-widest mb-8 text-center px-6 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>У цій команді ще немає гравців. {'\n'}Додайте їх, щоб почати тест.</Text>
                    <TouchableOpacity onPress={onBack} className={`border py-4 px-8 rounded-2xl shadow-sm ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-300'}`}>
                        <Text className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Повернутися назад</Text>
                    </TouchableOpacity>
                </View>
            </View>
        );
    ...
    return (
        <View className="flex-1 px-4 pt-4">
            <View className="flex-row items-center justify-between mb-2">
                <TouchableOpacity onPress={onBack} className={`p-2 -ml-2 rounded-full ${isDark ? 'active:bg-slate-800' : 'active:bg-slate-200'}`}><Feather name="chevron-left" size={28} color={isDark ? "white" : "black"} /></TouchableOpacity>
                <Text className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Оберіть гравців</Text>
                <View className="w-10" />
            </View>

            <View className="flex-row justify-between items-center mb-4">
                <Text className={`font-bold text-base ${isDark ? 'text-blue-500' : 'text-blue-600'}`}>ОБРАНО: <Text className={isDark ? 'text-white' : 'text-slate-900'}>{selectedIds.length}</Text> з {players.length}</Text>
            </View>
            ...
            <View className="flex-row justify-between mb-4 px-1">
                <Text className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>Список гравців</Text>
                <TouchableOpacity onPress={toggleAll}>
                    <Text className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>{selectedIds.length === players.length && players.length > 0 ? 'Зняти вибір з усіх' : 'Обрати всіх'}</Text>
                </TouchableOpacity>
            </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        ...
                        return (
                            ...
                            >
                                <View className={`w-6 h-6 rounded-md items-center justify-center mr-4 ${
                                    isSelected
                                ...
                                <View>
                                    <Text className={`font-bold text-base ${isDark ? 'text-white' : 'text-slate-900'}`}>{item.name}</Text>
                                </View>
                            </TouchableOpacity>
                        );
                    }}
            ...
            <TouchableOpacity
                ...
                className={`w-full py-5 rounded-2xl items-center mb-8 absolute bottom-0 left-4 right-4 shadow-lg ${
                    selectedIds.length > 0
                        ? (isDark ? 'bg-yellow-400 shadow-yellow-400/20' : 'bg-yellow-400 shadow-yellow-400/30')
                        : (isDark ? 'bg-slate-900 border border-slate-800 opacity-50' : 'bg-slate-200 opacity-70')
                }`}
            >
                <Text className={`font-black text-lg uppercase ${selectedIds.length > 0 ? 'text-slate-900' : (isDark ? 'text-slate-600' : 'text-slate-400')}`}>Продовжити</Text>
            </TouchableOpacity>
        </View>
    );
}