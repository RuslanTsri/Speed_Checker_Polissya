...
const SensorDot = React.memo(({ isTriggered, isStart, isFinish, isDark }: { isTriggered: boolean, isStart: boolean, isFinish: boolean, isDark: boolean }) => {
...
export default function SpeedTestRun({ config, onBack, onFinish }: { config: any, onBack: () => void, onFinish: () => void }) {
    ...
    const {
        ...
        isRunning, isFinished, isReady, timeObj, progressPercent, activeSensors,
    ...
    const colors = { bgList: isDark ? 'rgba(2, 6, 23, 0.5)' : '#f8fafc', borderList: isDark ? '#1e293b' : '#e2e8f0', borderRow: isDark ? 'rgba(30, 41, 59, 0.5)' : '#f1f5f9', textMain: isDark ? '#ffffff' : '#0f172a', textSub: isDark ? '#64748b' : '#64748b', yellow: isDark ? '#facc15' : '#eab308' };
    ...
    return (
        <View className={`flex-1 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
            <ScrollView className="flex-1 pt-4 px-4" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
                <View className="flex-row items-center justify-between mb-6">
                    ...
                    <View className="items-center">
                        <Text className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('tools.speed_checker.testing_title') as string}</Text>
                ...
                <View className={`border rounded-2xl p-4 mb-8 flex-row justify-between items-center shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                    <View className="flex-row items-center">
                        ...
                        <View>
                            <Text className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{t('tools.speed_checker.player_index', { current: currentPlayerIndex + 1, total: totalPlayers }) as string}</Text>
                ...
                <View className="items-center mb-12">
                    <Text className={`text-xs font-bold tracking-[0.3em] uppercase mb-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{isFinished ? (t('tools.speed_checker.result') as string) : (t('tools.speed_checker.time') as string)}</Text>
                ...
                <View className="mb-12 h-80 relative w-full items-center">
                    ...
                    {activeSensors.map((sensor, index) => {
                        ...
                        const isStart = index === 0; const isFinish = index === activeSensors.length - 1;
                        const top = (index / (Math.max(1, activeSensors.length - 1))) * 100;
                        let label = `GATE ${index}`; if (isStart) label = "START"; else if (isFinish) label = "FINISH";
                        let timeDisplay = "--:--";
                        if (isStart && (isRunning || isFinished)) timeDisplay = "00:00.00";
                        else if (sensor.triggerTime) { const t = formatTime(sensor.triggerTime / 1000); timeDisplay = `${t.main}${t.decimal}`; }
                        return (
                ...
                <View className="mb-6">
                    {!isRunning && !isFinished && !isReady ? (
                        <TouchableOpacity onPress={startTraining} className="bg-green-500 py-5 rounded-2xl items-center flex-row justify-center shadow-sm">
                            <Feather name="play" size={24} color="#0f172a" style={{ marginRight: 10 }} /><Text className="font-black text-xl text-slate-900">{t('tools.speed_checker.btn_start') as string}</Text>
                        </TouchableOpacity>
                    ) : isRunning ? (
                        <TouchableOpacity onPress={stopTraining} className="bg-red-500 py-5 rounded-2xl items-center flex-row justify-center shadow-sm">
                            <Feather name="square" size={24} color="white" style={{ marginRight: 10 }} /><Text className="text-white font-black text-xl">{t('tools.speed_checker.btn_stop') as string}</Text>
                        </TouchableOpacity>
                    ) : !isFinished ? (
                        <View className="flex-row space-x-3">
                            <TouchableOpacity onPress={resetSession} className={`flex-1 py-4 rounded-2xl items-center mr-2 border shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}><Text className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{t('tools.speed_checker.btn_reset') as string}</Text></TouchableOpacity>
                            <TouchableOpacity onPress={nextPlayer} className="flex-[2] bg-yellow-400 py-4 rounded-2xl items-center shadow-sm"><Text className="text-slate-900 font-black text-lg uppercase">{t('tools.speed_checker.btn_skip') as string}</Text></TouchableOpacity>
                        </View>
                    ) : null}
                </View>
            </ScrollView>
            ...
            <AppModal visible={showIndividualModal} onClose={() => {}} title={t('tools.speed_checker.modal_result', { name: currentPlayerObj.name }) as string} type="center">
                <View className="items-center">
                    <Text className={`text-xs font-bold uppercase tracking-widest mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('tools.speed_checker.run_time') as string}</Text>
                    ...
                    <View className="flex-row w-full gap-3">
                        <TouchableOpacity onPress={retryIndividualRun} className={`flex-1 py-4 rounded-xl items-center border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-300'}`}><Text className={`font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('tools.speed_checker.btn_retry') as string}</Text></TouchableOpacity>
                        <TouchableOpacity onPress={confirmIndividualRun} className="flex-1 bg-green-500 py-4 rounded-xl items-center flex-row justify-center"><Text className="text-slate-900 font-black uppercase mr-1">{t('tools.speed_checker.btn_accept') as string}</Text><Feather name="check" size={18} color="#0f172a" /></TouchableOpacity>
                    </View>
                </View>
            </AppModal>
            ...
            <AppModal visible={showSummaryModal} onClose={() => {}} title={t('tools.speed_checker.modal_summary', { count: localResults.length }) as string} type="center">
                <View className="h-96 w-full">
                    <Text className={`text-xs mb-4 text-center ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{t('tools.speed_checker.summary_desc') as string}</Text>
                    <FlatList
                        data={localResults} keyExtractor={(_, index) => index.toString()}
                        className="flex-1 rounded-xl border mb-4" style={{ backgroundColor: colors.bgList, borderColor: colors.borderList }} contentContainerStyle={{ padding: 12 }} showsVerticalScrollIndicator={false}
                        renderItem={({ item, index }) => (
                            <View className="flex-row justify-between items-center py-3 border-b" style={{ borderColor: colors.borderRow }}>
                                <View className="flex-row items-center"><Text className="w-6 font-bold" style={{ color: colors.textSub }}>{index + 1}.</Text><View><Text className="font-bold text-sm" style={{ color: colors.textMain }}>{item.player.name}</Text><Text className="text-[10px]" style={{ color: colors.textSub }}>#{item.player.number}</Text></View></View>
                    ...
                    <View className="flex-row w-full gap-3 mt-2">
                        <TouchableOpacity onPress={restartWholeSession} className={`flex-1 py-4 rounded-xl items-center border ${isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'}`}><Text className={`font-bold uppercase ${isDark ? 'text-red-400' : 'text-red-500'}`}>{t('tools.speed_checker.btn_reset_all') as string}</Text></TouchableOpacity>
                        <TouchableOpacity onPress={saveAllResults} disabled={isSaving} className="flex-1 bg-green-500 py-4 rounded-xl items-center flex-row justify-center">
                            {isSaving ? <ActivityIndicator color="#0f172a" size="small" /> : <><Text className="text-slate-900 font-black uppercase mr-1">{t('tools.speed_checker.btn_save_db') as string}</Text><Feather name="database" size={18} color="#0f172a" /></>}
                        </TouchableOpacity>
                    </View>
                </View>
            </AppModal>
